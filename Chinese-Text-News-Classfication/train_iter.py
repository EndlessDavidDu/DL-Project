...
def train(config, model, data, name, test_dataloader, eva_dataloader, logger):
    ...
    logger.add_log(f'SummaryWriter is ready，The saving address is "./log/SummaryWriter', 'debug')
    ...
    logger.add_log(f'Loading complete, it saved as：{config.model_save_path} ', 'debug')

    report, accurate, f1 = test(model=model, data=test_dataloader)
    logger.add_log(
        f'Result: accurate: {accurate}, f1-score: {f1}, report: \n {report}', 'debug')
    print(report)
